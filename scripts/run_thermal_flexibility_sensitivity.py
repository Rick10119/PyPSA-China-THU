...
def _plot_threshold_comparison(output_root: Path, thresholds: list[float]) -> None:
    ...
    for threshold in thresholds:
        workbook = output_root / f"threshold_{_tag(threshold)}" / "solar_value_dataset.xlsx"
        if not workbook.is_file():
            continue
        series = _national_weighted_series(workbook)
        series.insert(0, "threshold", float(threshold))
        ...
        ax.plot(
            ...
            marker="o",
            linewidth=2.0,
            label=f"min output = {threshold:g}",
        )
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="thermal-flex-config-", dir=config_path.parent) as tmp:
        ...
        for threshold in thresholds:
            tag = _tag(threshold)
            case_dir = output_root / f"threshold_{tag}"
            ...
            pe["daily_low_output_zero_threshold"] = float(threshold)
            ...
            # This separate 10% rule otherwise makes threshold 0.1 and 0 identical.
            pe.setdefault("thermal_load_floor", {})["enabled"] = False
            case_config_path = tmp_dir / f"config_threshold_{tag}.yaml"
